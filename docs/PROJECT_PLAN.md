...
- `/admin-demo`를 좌측 편집 패널 + 우측 실시간 미리보기(iframe) 2열 구조로 구현했습니다.
- 편집 카테고리는 네비게이션, 섹션 순서, Hero, News, Subscribe, Product Tabs, Stats, Awards, Footer로 나누어 위 편집 항목 전체를 커버합니다.
- 반복 항목(뉴스/통계/수상/메뉴/제품 탭/구독 요소/섹션)은 위·아래 이동과 추가·삭제를 지원하며, 섹션은 노출 토글도 제공합니다.
- 편집 상태는 `src/lib/content/admin-store.ts`가 `localStorage` + `BroadcastChannel` + `storage` 이벤트로 관리하며, `useSyncExternalStore`로 편집기와 미리보기가 같은 스냅샷을 구독합니다.
- 미리보기 `/admin-demo/preview`는 실제 공개 페이지 컴포넌트(`SiteHeader`, `SectionRenderer`, `SiteFooter`)를 그대로 재사용하고, 편집 즉시 반영됩니다. 데스크톱/태블릿/모바일 폭 토글을 제공합니다.
- 자동 저장(localStorage), 초기화(기본 JSON 복원), `demo-site.json` 내보내기를 지원합니다.
- 실제 DB 저장, 인증, 권한, 이메일 발송, 파일 업로드는 데모 범위에서 제외했습니다.
...
- 3단계에서 만든 `/admin-demo` 좌측 속성 패널을 멀티 페이지 관리와 아코디언 편집 구조로 개편한다.
- 미리보기에서 현재 편집 중인 섹션/전역 요소를 오버레이로 하이라이트한다.

구현 방향:

- 상단 카테고리 탭을 "편집 대상" 드롭다운으로 대체한다. 드롭다운은 `공통 요소`를 맨 위에 두고, 그 아래에 페이지 목록(현재는 `홈`)을 나열한다.
- 선택한 대상의 섹션들을 아코디언으로 펼쳐 한 화면에서 편집한다.
- 편집 중인 항목에 대응하는 미리보기 영역을 오버레이로 하이라이트하고 해당 위치로 스크롤한다. 헤더/푸터 등 전역 요소도 동일하게 하이라이트한다.
- 편집기→미리보기 하이라이트 통신은 콘텐츠 스토어와 분리해 iframe `postMessage`로 처리한다.
- 멀티 페이지 데이터 모델(`pages` 맵 → 배열) 전환은 후속 Phase로 분리한다.
